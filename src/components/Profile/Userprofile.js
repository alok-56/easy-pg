...
function MyVerticallyCenteredModal(props) {
  ...
  const user = async () => {
    let data = await fetch(
      `https://easy-ser.vercel.app/Aut/singleuser/${userid}`
    );
  ...
  const updateuser = async () => {
    let data = await fetch(
      `https://easy-ser.vercel.app/Aut/profile/${userid}`,
      {
        method: "put",
        body: JSON.stringify({
          name,
  ...
  return (
    ...
    >
      ...
      <Modal.Footer>
        {spin ? <Button onClick={updateuser}>Confirm</Button> : null}
      </Modal.Footer>
    </Modal>
  );
}
              ...
              <div style={{ display: "flex", justifyContent: "center" }}>
                <div style={{ display: "block" }}>
                  <img
                    width={150}
                    src={images}
                    style={{ borderRadius: "50%", maxHeight: "200px" }}
                    alt="profile"
                  ></img>
                </div>
                <div className="m-3">
